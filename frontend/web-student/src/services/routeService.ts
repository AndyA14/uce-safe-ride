...
const API_URL =
  import.meta.env.VITE_API_URL || 'http://localhost:8003/api/v1/routes';
...
/**
 * Detalle completo de una ruta (incluye polyline)
 */
...
export const getActiveRoute = async (
  token: string
): Promise<ActiveRouteResponse | null> => {
  try {
    const response = await axios.get<ActiveRouteResponse>(
      `${API_URL}/active/me`,
      {
        headers: {
...
export const getRoutes = async (): Promise<Route[]> => {
  ...
  try {
    ...
    return response.data.map((backendRoute, index) => ({
      ...
      active: backendRoute.active ?? true,
...
/**
 * Obtiene una ruta por ID (SIN polyline, solo datos UI)
 */
...
export const getRouteDetails = async (
  ...
  try {
    ...
    console.error(`Error obteniendo ruta ${routeId}:`, error);
    throw error;
  }
};

/**
 * Obtiene las paradas asociadas a una ruta
 */
export const getRouteStops = async (
  ...
  return new Promise((resolve) =>
    setTimeout(() => resolve(mockStops), 300)
  );
};
 ...
 */
export const getRouteBuses = async (
  routeId: string
): Promise<Bus[]> => {
  return new Promise((resolve) =>
...
/**
 * Obtiene paradas cercanas a una ubicación
 */
...
export const getBusLocation = async (
  ...
  return new Promise((resolve) =>
    setTimeout(
      () => resolve({ lat: -0.2105, lng: -78.4917 }),
      300
    )
  );
};
